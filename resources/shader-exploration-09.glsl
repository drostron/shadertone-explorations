...
void main( void )
{
  float vol = iOvertoneVolume;
    ...
    // camera
    vec3 ro = 2.0*vec3(cos(0.3*iGlobalTime),2.0*sin(0.7*iGlobalTime),2.0*sin(1.1*iGlobalTime));
    // vec3 ro = 2.0*vec3(1.0,1.0,5.0);
    vec3 ta = vec3(2.0,2.0,2.0);
    // camera matrix
    vec3 ww = normalize( ta - ro );
    vec3 uu = normalize( cross(ww,vec3((10*vol),1.0,0.0+(10*vol)) ) );
	...
	{
		// position disk
    vec3 r = (3.5-(50*vol))*(-1.0 + 2.0*hash3( float(i) ));
